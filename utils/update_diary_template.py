...
class Tex2TexConverter:
    ...
    def _load_template(self) -> str:
        ...
        template_path = Path(__file__).parent / "assets" / "templates" / "entries" / "entry_template.tex"
        try:
    ...
    def inject_personal_info(self, template: str, tex_file: Path, folder_name: str) -> str:
    ...
    def copy_assets(self, tex_file: Path, output_dir: Path, use_symlinks: bool = False) -> List[Path]:
    ...
    def convert_file(self, tex_file: Path, output_dir: Path, folder_name: str, use_symlinks: bool = False) -> bool:
        ...
        try:
            ...
            if '\\bibliographystyle' in template:
                ...
                new_content = template[:bib_start] + content + '\n\n' + template[bib_start:]
            else:
    ...
    def convert_folder(self, input_folder: Path, output_folder: Optional[Path] = None, use_symlinks: bool = False) -> int:
...
Examples:
  ...
  ./tex2tex_convert.py posts/2024/ --verbose
    ...
    parser.add_argument('input_folder', type=Path, help='Input folder containing .tex files')
    parser.add_argument('output_folder', type=Path, nargs='?', help='Output folder (default: converted_<input_folder_name>)')
